...
def main():
    ...
    y_train = pd.Series(labels_train, dtype=np.int32) - 1
    ...
    # Calculate the f1 score and accuracy of training set
    probs_train = gbm.predict(X_train)
    preds_train = np.argmax(probs_train, axis=1)
    score_train = f1_score(y_train, preds_train, average='weighted')
    acc_train = accuracy_score(y_train, preds_train)
    print("The f1 score of training set after %d epochs is: %f" % (num_boost_round, score_train))
    print("The accuracy of training set after %d epochs is: %f" % (num_boost_round, acc_train))

    print("Save model...")
    gbm.save_model("2018-07-15_lgb-char-300d.txt")
    ...
    submission.to_csv("../../submissions/2018-07-15_lgb-char-300d-submission.csv", index=False)
