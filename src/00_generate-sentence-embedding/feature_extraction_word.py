...
"""
Generate feature vectors of sentences by summing or averaging the word embeddings.
...
def generate_word_mapping(word_vectors_path):
    """Generate the mapping from words to its corresponding vectors."""
    word_to_vec_map = {PAD_STR: np.zeros(EMBEDDING_SIZE, dtype=np.float32)}
    with open(word_vectors_path, 'r', encoding='utf-8') as f:
        ...
        for line in lines:
            ...
            if word not in word_to_vec_map:
                word_to_vec_map[word] = np.array(line[1:], dtype=np.float32)
    return word_to_vec_map


def generate_features_sum(sample, word_to_vec_map):
...
def plan1():
    ...
    Despite the length of the document, summing all the word vectors
    ...
    # Process each sample to a fixed length (optional)
    # ============================================================================================
    ...
    word_vectors_path = "../../word_vectors/gensim-word-300d.txt"
    word_to_vec_map = generate_word_mapping(word_vectors_path)

    # Generate feature vectors and save
    # ============================================================================================
    ...
    for word_sample in word_samples:
        feature_vector = generate_features_sum(word_sample, word_to_vec_map)
    ...
    X_train.to_csv("../../processed_data2/train-data-word-300d-sum.txt", index=False)
    X_test.to_csv("../../processed_data2/test-data-word-300d-sum.txt", index=False)
...
def plan2():
    ...
    # Save to CSV file
    print("Saving to csv file...")
    X_train.to_csv("../../processed_data2/train-data-word-300d-mean.txt", index=False)
    X_test.to_csv("../../processed_data2/test-data-word-300d-mean.txt", index=False)
